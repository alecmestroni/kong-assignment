...
Cypress.Commands.add("openWorkspaceOverview", () => {
  cy.getDataTestId(`${Cypress.env("SELECTORS").WORKSPACE_LINK_PREFIX}${Cypress.env("WORKSPACE_NAME")}`).click()
  cy.url().should("include", `/${Cypress.env("WORKSPACE_NAME")}${Cypress.env("PATHS").OVERVIEW}`)
...
Cypress.Commands.add("selectInputFromWrapper", (dataTestId, id) => {
  ...
  cy.getDataTestId(`${Cypress.env("SELECTORS").SELECT_ITEM_PREFIX}${id}`).click()
})
...
Cypress.Commands.add("saveTextFromInput", (dataTestId, envVarName) => {
  cy.getDataTestId(dataTestId)
    ...
    .then((text) => {
      ...
      cy.log(`${envVarName.toUpperCase()} saved to env: ${Cypress.env(envVarName)}`)
    })
})
...
Cypress.Commands.add("checkTextFromCopy", (dataTestId, envVarName) => {
  cy.getDataTestId(dataTestId)
    ...
    .invoke("text")
    .then((text) => {
      expect(text, `Text: ${text} should match ${envVarName}: ${Cypress.env(envVarName)}`).to.eq(Cypress.env(envVarName))
    })
})

Cypress.Commands.add("interceptSimple", (path, method = "POST", isNested = true) => {
...
Cypress.Commands.add("interceptAndRemoveProp", (path, propToRemove, method = "POST", isNested = true) => {
  if (!path) throw new Error("path is required")
  ...
  cy.intercept(
    ...
    (req) => {
      if (req.body && typeof req.body === "object" && propToRemove in req.body) {
...
Cypress.Commands.add("waitSimple", (name, statusCode = 200) => {
  cy.wait(`@${name}`, { timeout: 10_000 }).then((api) => {
    ...
    expect(api.response.statusCode, `api should response with status: ${statusCode}`).to.eq(statusCode)
    return api
  })
})

Cypress.Commands.add("clickDataTestIdAndWaitApi", (dataTestId, path, method = "POST", statusCode = 200, isNested = true) => {
