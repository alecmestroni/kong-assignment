describe("Gateway Service Creation - URL Form", () => {
  ...
  describe("Form State and Validation", () => {
    it("should keep submit button disabled when no URL is provided", { jiraKey: "KONG-1583" }, () => {
      cy.getDataTestIdDisabled(Cypress.env("SELECTORS").SERVICE_FORM_SUBMIT)
    })

    it("should show browser validation error when URL field is empty", { jiraKey: "KONG-8924" }, () => {
      ...
      cy.getDataTestId(Cypress.env("SELECTORS").SERVICE_FORM_INPUT_URL).invoke("prop", "validity").its("valueMissing").should("be.true")
    ...
    it("should not send validation request when required attribute is removed", { jiraKey: "KONG-5076" }, () => {
      cy.removeAttrFromDataTestId(Cypress.env("SELECTORS").SERVICE_FORM_SUBMIT, "disabled")
      cy.removeAttrFromDataTestId(Cypress.env("SELECTORS").SERVICE_FORM_INPUT_URL, "required")
  ...
  describe("Server-side Validation", () => {
    it("should show server error when URL property is missing from request", { jiraKey: "KONG-2368" }, () => {
      cy.getDataTestId(Cypress.env("SELECTORS").SERVICE_FORM_INPUT_URL).type(Cypress.env("SERVICE_URL"))
  ...
  describe("Successful Service Creation", () => {
    it("should create Gateway Service successfully with valid URL", { jiraKey: "KONG-7451" }, () => {
      cy.getDataTestId(Cypress.env("SELECTORS").SERVICE_FORM_INPUT_URL).type(Cypress.env("SERVICE_URL"))
