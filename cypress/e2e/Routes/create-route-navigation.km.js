describe("Gateway Route Creation - Navigation Flow", () => {
  ...
  describe("Gateway Service Page - Alert Button", () => {
    it("should redirect to route creation form with pre-selected service and redirect URL", { jiraKey: "KONG-5784" }, () => {
      ...
      cy.get(Cypress.env("SELECTORS").ROUTE_SERVICE_SELECT_INPUT).should("not.exist")
      cy.contains(Cypress.env("LABELS").ROUTE_CREATION).should("exist")
    })
  })
  describe("Workspace Overview - Action Button", () => {
    it("should redirect to route creation form with service selector visible and CTA parameter", { jiraKey: "KONG-3921" }, () => {
      cy.visitHomePage()
